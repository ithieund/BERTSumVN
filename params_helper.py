...
def _parse_arguments():
	...
	if 'train_model' in executed_script_name or 'evaluate_model' in executed_script_name:
		...
		if 'train_model' in executed_script_name:
			...
			parser.add_argument('-train_batch_size', type=int, default=32, help='Set larger batch size if we have large GPU memory')
			parser.add_argument('-valid_batch_size', type=int, default=32, help='Set larger batch size if we have large GPU memory')
			parser.add_argument('-num_train_epochs', type=int, default=50, help='Max number of training epochs')
			parser.add_argument('-learning_rate', type=float, default=0.00005, help='Peak learning rate after warmup. Set 0 to disable warmup')
			parser.add_argument('-num_warmup_steps', type=int, default=0, help='Num optimization steps to do warmup before applying learning rate schedule.')
			parser.add_argument('-gradient_accumulation_steps', type=int, default=2, help='Instead of calculating gradient and backpropagating it at every training step, we will wait until it reach the accummulation step to do that kind of calculation at once, which will speed up training a bit. But it will change the definition of the optimization step: num optimization steps = num steps / num gradient acc steps')
			...
			parser.add_argument('-last_best_eval_loss', type=float, default=None, help='Provide last best eval loss when resume training from last epoch. Otherwise, leave default value None')
			parser.add_argument('-early_stopping_delta', type=float, default=0)
			parser.add_argument('-early_stopping_patience', type=int, default=5, help='How many epochs to wait for early stopping when the current epoch has worst performance. Set 0 to disable early stopping')
			...
			parser.add_argument('-save_total_limit', type=int, default=1, help='How many checkpoints to save at max')
		...
		if 'evaluate_model' in executed_script_name:
			...
			parser.add_argument('-n_best', type=int, default=3, help='Final best hypotheses to return. Should be less than or equal to beam_size')
			parser.add_argument('-min_tgt_len', type=int, default=20, help='Define min output sequence length to prevent too short sequence. Set 0 to disable this rule')
	...
	if params.quick_test:
		if 'script_train_model' in executed_script_name:
			...
			setattr(params, 'early_stopping_delta', 0.0)
